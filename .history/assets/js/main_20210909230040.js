...
Validator.DangKyLan2=function(selector,getThong,message){
    return {
        selector:selector,
        test:function(value){
            var a= String(getThong())
            console.log(a)
            for (var i in a){
                console.log(a[i])
                return value != a[i] ? undefined : message|| 'Email này đã dùng cho tài khoản khác'
            }
           // return value == getThong() ? undefined : message|| 'Email này đã dùng cho tài khoản khác'
        }
    }
}